    ...
    try {
        $stmt_cliente_contexto = $pdo->prepare(
            "SELECT c.id_empresa_master, w.slug, w.nome_empresa, w.cor_variavel
             FROM kyc_clientes c
             LEFT JOIN configuracoes_whitelabel w ON c.whitelabel_parceiro_id = w.id
             WHERE c.id = ?"
        );
        ...
        if ($cliente_contexto) {
            ...
            // Caso de erro de integridade de dados.
            throw new Exception("Seus dados de cliente não estão associados a nenhuma empresa. Por favor, contate o suporte.");
        }
...
if (empty($cor_variavel_contexto) && !empty($slug_contexto)) {
    try {
        $stmt = $pdo->prepare("SELECT cor_variavel, nome_empresa, id_empresa_master FROM view_whitelabel_context WHERE slug = ?");
        ...
        if ($config_reforco) {
            ...
            if (empty($id_empresa_master_contexto)) $id_empresa_master_contexto = $config_reforco['id_empresa_master'];
        }
