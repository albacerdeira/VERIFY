...
<?php
// --- CORREÇÃO: Definição robusta da cor variável ---
$cor_final_whitelabel = '';
if (!empty($slug_contexto)) {
    try {
        ...
        $config = $stmt->fetch();
        if ($config && !empty($config['cor_variavel'])) {
            $cor_final_whitelabel = $config['cor_variavel'];
        }
    } catch (PDOException $e) {
        error_log("Erro ao buscar cor variável: " . $e->getMessage());
    }
}

// Fallback para cor padrão se não encontrar no banco
if (empty($cor_final_whitelabel)) {
    $cor_final_whitelabel = '#4f46e5'; // Cor padrão
}
?>

<style>
    /* --- CORREÇÃO: Definição explícita da cor dinâmica --- */
    :root { 
        --primary-color: <?= htmlspecialchars($cor_final_whitelabel) ?>; 
    }
    /* --- CORREÇÃO: Uso direto da variável CSS em todos os lugares relevantes --- */
...
<div class="container bg-white p-4 p-md-5 rounded shadow-sm">
    ...
    <form action="kyc_submit.php" method="POST" enctype="multipart/form-data" id="kyc-form" novalidate>
        ...
        <!-- O restante do seu formulário (HTML e JavaScript) do arquivo "kyc_form atual.php" continua aqui, sem nenhuma alteração. -->
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let currentStep = 0;
    const STORAGE_KEY = 'kycFormData_v5.1';
    ...
    function validateCurrentStep() {
        ...
        for (const element of requiredElements) {
            ...
            if (type === 'file') {
                if (element.files.length === 0) isElementInvalid = true;
            } else if (type === 'checkbox') {
                if (!element.checked) isElementInvalid = true;
            } else {
                if (element.value.trim() === '') {
                    isElementInvalid = true;
                } 
                else if (element.classList.contains('ubo-cpf-cnpj')) {
                    const rawValue = element.value.replace(/\D/g, '');
                    if (rawValue.length > 0 && rawValue.length !== 11 && rawValue.length !== 14) {
                        isElementInvalid = true;
                        const feedback = element.nextElementSibling;
                        if (feedback && feedback.classList.contains('invalid-feedback')) {
                            feedback.textContent = 'Documento incompleto. Deve ter 11 (CPF) ou 14 (CNPJ) dígitos.';
                        }
                    }
                }
            }
        ...
        if (currentStep === 1) { 
            ...
            if (checkboxGroup) {
                ...
                if (checkedCheckboxes.length === 0) {
                    ...
                    isValid = false;
                    if (!firstInvalidElement) firstInvalidElement = checkboxGroup.querySelector('input');
                } else {
    ...
    function saveFormData() {
        ...
        formData.forEach((value, key) => {
            ...
            } else if (!key.startsWith('socios[')) {
                object[key] = value;
            }
        });

        const socios = [];
        const uboCards = uboContainer.querySelectorAll('.ubo-card');
        uboCards.forEach((card) => {
            const socioData = {};
            ...
            inputs.forEach(input => {
                ...
                if (match) {
                    ...
                    if (input.type === 'checkbox') {
                        socioData[key] = input.checked ? '1' : '0';
                    } else if (input.type !== 'file') {
                        socioData[key] = input.value;
                    }
                }
            });
        ...
        for (const key in data) {
            ...
            if (element.type === 'checkbox') {
                 if (elements.length > 1) {
                    ...
                    document.querySelectorAll(`input[name="${key}[]"]`).forEach(chk => {
                ...
                document.querySelectorAll(`input[name="${key}"]`).forEach(rad => {
                    if(rad.value === value) rad.checked = true;
                });
            ...
            data.socios.forEach((socioData, index) => {
                ...
                if (newCard) {
                    for (const key in socioData) {
                        ...
                        if (input) {
                            if (input.type === 'checkbox') {
                                input.checked = socioData[key] === '1';
                            } else {
                                input.value = socioData[key];
                            }
