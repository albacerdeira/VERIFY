...
// --- LÓGICA WHITELABEL MELHORADA ---

// Define os valores padrão da sua plataforma.
$nome_empresa_padrao = 'Verify KYC';
$cor_variavel_padrao = '#4f46e5'; // Cor padrão da Verify KYC
$logo_url_padrao = 'imagens/verify-kyc.png'; // Logo padrão
...
// Verifica se há um slug de cliente na URL para carregar a configuração whitelabel.
...
if ($slug_contexto && isset($pdo)) {
    try {
        ...
        $config = $stmt->fetch();
        if ($config) {
            // Se encontrou uma configuração para o slug, substitui os valores padrão.
            $nome_empresa = $config['nome_empresa'];
            $cor_variavel = $config['cor_variavel'];
            $logo_url = $config['logo_url'];
        }
    } catch (PDOException $e) {
        // Em caso de erro, apenas registra e continua com os valores padrão.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $pdo->prepare("SELECT id, nome_completo, password, email, email_verificado, status FROM kyc_clientes WHERE email = ?");
        $stmt->execute([$email]);
        $cliente = $stmt->fetch();
        ...
        // Constrói a URL do dashboard, incluindo o slug do cliente se ele existir.
        $redirect_url = 'cliente_dashboard.php';
        if ($slug_contexto) {
            $redirect_url .= '?cliente=' . urlencode($slug_contexto);
        }
...
// Captura e limpa mensagens da sessão
$success = $_SESSION['success_message'] ?? '';
$error_from_session = $_SESSION['error_message'] ?? '';
if ($error_from_session) {
